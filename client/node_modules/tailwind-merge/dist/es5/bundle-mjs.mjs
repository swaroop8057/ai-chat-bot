 ...
 */
var concatArrays = function concatArrays(array1, array2) {
  ...
  for (var _i = 0; _i < array2.length; _i++) {
    combinedArray[array1.length + _i] = array2[_i];
  }
  return combinedArray;
};

// Factory function ensures consistent object shapes
var createClassValidatorObject = function createClassValidatorObject(classGroupId, validator) {
  return {
...
var createClassPartObject = function createClassPartObject(nextPart, validators, classGroupId) {
  if (nextPart === void 0) {
    nextPart = new Map();
  }
...
var createClassGroupUtils = function createClassGroupUtils(config) {
  ...
  var getClassGroupId = function getClassGroupId(className) {
    ...
    return _getGroupRecursive(classParts, startIndex, classMap);
  };
...
var _getGroupRecursive = function getGroupRecursive(classParts, startIndex, classPartObject) {
  ...
  if (nextClassPartObject) {
    var result = _getGroupRecursive(classParts, startIndex + 1, nextClassPartObject);
...
var processClassGroups = function processClassGroups(classGroups, theme) {
...
var processClassesRecursively = function processClassesRecursively(classGroup, classPartObject, classGroupId, theme) {
...
var processClassDefinition = function processClassDefinition(classDefinition, classPartObject, classGroupId, theme) {
...
var processStringDefinition = function processStringDefinition(classDefinition, classPartObject, classGroupId) {
  var classPartObjectToEdit = classDefinition === '' ? classPartObject : getPart(classPartObject, classDefinition);
  classPartObjectToEdit.classGroupId = classGroupId;
};
var processFunctionDefinition = function processFunctionDefinition(classDefinition, classPartObject, classGroupId, theme) {
...
var processObjectDefinition = function processObjectDefinition(classDefinition, classPartObject, classGroupId, theme) {
...
var createResultObject = function createResultObject(modifiers, hasImportantModifier, baseClassName, maybePostfixModifierPosition, isExternal) {
  return {
    modifiers: modifiers,
    ...
    for (var i = 0; i < modifiers.length; i++) {
      ...
      if (isArbitrary || isOrderSensitive) {
        ...
        if (currentSegment.length > 0) {
          currentSegment.sort();
          result.push.apply(result, currentSegment);
          currentSegment = [];
        }
...
var twJoin = function twJoin() {
  ...
  while (index < arguments.length) {
    var _index;
    if (argument = (_index = index++, _index < 0 || arguments.length <= _index ? undefined : arguments[_index])) {
...
var createTailwindMerge = function createTailwindMerge(createConfigFirst) {
  ...
  var cacheSet;
  var functionToCall;
  var initTailwindMerge = function initTailwindMerge(classList) {
  ...
  var tailwindMerge = function tailwindMerge(classList) {
  ...
  return function () {
    return functionToCall(twJoin.apply(void 0, arguments));
  };
};
